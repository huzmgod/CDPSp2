...
if __name__ == '__main__':
    group = os.environ.get('GROUP_NUMBER')
    path = os.path.dirname(os.path.abspath(__file__))
    with open(f"{path}/templates/index.html", "r") as index, \
         open(f"{path}/templates/index~.html", "a") as newIndex:
        for line in index.readlines():
            newLine = f"{{% block title %}}{group} Bookstore App{{% endblock %}}\n" \
                    if "{% block title %}" in line else line
            newIndex.write(newLine)
        os.system(f"mv {path}/templates/index~.html {path}/templates/index.html")

    with open(f"{path}/templates/productpage.html", "r") as index, \
         open(f"{path}/templates/productpage~.html", "a") as newIndex:
        for line in index.readlines():
            newLine = f"{{% block title %}}{group} Bookstore App{{% endblock %}}\n" \
                    if "{% block title %}" in line else line
            newIndex.write(newLine)
        os.system(f"mv {path}/templates/productpage~.html {path}/templates/productpage.html")
